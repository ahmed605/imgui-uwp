...
{
	void MainPage::swapchain_Loaded(winrt::Windows::Foundation::IInspectable const& sender, winrt::Windows::UI::Xaml::RoutedEventArgs const& e)
	{
		...
		ImGui_ImplUwp_InitForSwapChainPanel(get_abi(swapChain));
	...
	void MainPage::InitDevice(winrt::Windows::UI::Core::CoreWindow const& window, const winrt::Windows::UI::Xaml::Controls::SwapChainPanel& panel)
	{
		window.Closed({ this, &MainPage::OnWindowClosed });
		window.VisibilityChanged({ this, &MainPage::OnVisibiltyChanged });
		panel.SizeChanged({ this, &MainPage::OnSizeChanged });
		...
		swapChainDesc.Width = panel.ActualWidth() * m_scaleY;
		swapChainDesc.Height = panel.ActualHeight() * m_scaleX;
	...
	void MainPage::OnSizeChanged(IInspectable const&, winrt::Windows::UI::Xaml::SizeChangedEventArgs const& args)
	{
	...
	void MainPage::OnCompositionScaleChanged(const winrt::Windows::UI::Xaml::Controls::SwapChainPanel&, const IInspectable&)
	{
		...
		auto panel = swapchain();

		m_ResizeWidth = panel.ActualWidth();
		m_ResizeHeight = panel.ActualHeight();
	}
