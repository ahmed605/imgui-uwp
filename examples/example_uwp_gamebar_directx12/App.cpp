...
using namespace example_uwp_gamebar_directx12;
using namespace example_uwp_gamebar_directx12::implementation;
    ...
    if (rootFrame == nullptr)
    {
        ...
        if (e.PrelaunchActivated() == false)
        {
            if (rootFrame.Content() == nullptr)
            {
                ...
                // parameter
                rootFrame.Navigate(xaml_typename<example_uwp_gamebar_directx12::MainPage>(), box_value(e.Arguments()));
            }
...
void App::OnActivated(IActivatedEventArgs const& e)
{
    ...
    if (widgetArgs)
    {
        if (widgetArgs.IsLaunchActivation())
        {
            ...
            rootFrame.Navigate(xaml_typename<example_uwp_gamebar_directx12::MainPage>());
            Window::Current().Activate();

            m_widgetWindowClosedHandlerToken = Window::Current().Closed({ get_weak(), &App::WidgetWindowClosedHandler });
        }
    }
}

void App::WidgetWindowClosedHandler(IInspectable const&, IInspectable const&)
{
    m_widget = nullptr;
    Window::Current().Closed(m_widgetWindowClosedHandlerToken);
}